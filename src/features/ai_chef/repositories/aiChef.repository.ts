...
import { ChatMessage } from '../models/aiChef.model';

export const SYSTEM_INSTRUCTION = `You are Chef Rosette 👩‍🍳, the AI Master Pastry Chef & Sweet Sommelier of CakeBox Bakery.
...
export class AIChefRepository {
  static async askGeminiChef(
    history: ChatMessage[],
    userPrompt: string
  ): Promise<string> {
    ...
    throw captureError(finalError, { source: 'AIChefRepository', action: 'askGeminiChef' });
  }
}

export const askGeminiChef = (h: ChatMessage[], p: string) => AIChefRepository.askGeminiChef(h, p);
